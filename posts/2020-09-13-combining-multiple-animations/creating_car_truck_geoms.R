...
# Generate a truck 'grob' using a baseline PNG

# truck.raster <- png::readPNG("C:/Users/Saba Ikhlaq/Downloads/truck4.png")
# 
# # The `grid` grob actually responsible for rendering our truck, 
# # combines our transparent truck elements with a background rectangle
# # for color/fill.
# 
# truckGrob <- function(x, y, length, width, gp) {
#   grid::grobTree(
#     grid::rectGrob(
#       x, y, hjust=.5, height=width, width=length,
#       gp = gp
#     ),
#     grid::rasterGrob(
#       truck.raster, x=x, y=y, hjust=.5, height=width, width=length
#     ) ) }
# # The `ggproto` object that maps our data to the `grid` grobs
# 
# Geomtruck <- ggplot2::ggproto("Geomtruck", ggplot2::Geom,
#                               # Generate grobs from the data, we have to reconvert length/width so
#                               # that the transformations persist
#                               
#                               draw_panel=function(self, data, panel_params, coords) {
...
#                                 colour = NA, fill = "grey35", size = 0.5, linetype = 1, alpha = NA
#                               ),
#                               # Use the truck grob in the legend
#                               
#                               draw_key = function(data, params, size) {
#                                 with(
#                                   data,
...
# ) {
#   layer(
#     data=data, mapping=mapping, geom=Geomtruck, position="identity",
#     stat="identity", show.legend = show.legend, inherit.aes = inherit.aes,
#     params=list(...)
#   )
# }
